...
def trainTest(classifier, X, Y, Xtest, Ytest, quiet=False):
    """
...
def trainTestSet(classifier, dataset, quiet=False):
    trainTest(classifier, dataset.X, dataset.Y, dataset.Xte, dataset.Yte, quiet)

def learningCurve(classifier, X, Y, Xtest, Ytest, quiet=False):
    """
    ...
    for i in range(1, M+1):    # loop over "skip lengths"
        ...
        (trAcc, teAcc, Ypred) = trainTest(classifier, Xtr, Ytr, Xtest, Ytest, quiet)
...
def learningCurveSet(classifier, dataset, quiet=False):
    return learningCurve(classifier, dataset.X, dataset.Y, dataset.Xte, dataset.Yte, quiet)

def hyperparamCurve(classifier, hpName, hpValues, X, Y, Xtest, Ytest, quiet=False):
    M = len(hpValues)
    trainAcc = zeros(M)
    testAcc  = zeros(M)
    for m in range(M):
        ...
        classifier.reset()
        (trAcc, teAcc, Ypred) = trainTest(classifier, X, Y, Xtest, Ytest, quiet)
...
def hyperparamCurveSet(classifier, hpName, hpValues, dataset, quiet=False):
    return hyperparamCurve(classifier, hpName, hpValues, dataset.X, dataset.Y, dataset.Xte, dataset.Yte, quiet)
...
def plotDataAndBoundary(X,Y, w, b):
    plt.figure()
    ...
    plt.plot( array([inBounds[0][0], inBounds[1][0]]), array([inBounds[0][1], inBounds[1][1]]), 'k-', linewidth=2 )
    #plt.show(False)
    #print axes
